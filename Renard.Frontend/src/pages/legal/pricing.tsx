...
export const PricingPage = () => {
  ...
  // Make sure to add this ID to your frontend .env file
  // Example: VITE_DODO_PRODUCT_PRO=prod_renard_pro
  const PRO_PRODUCT_ID =
    import.meta.env.VITE_DODO_PRODUCT_PRO || "prod_renard_pro";
  ...
  const handleUpgrade = async () => {
    ...
    if (!token) {
      // Pass 'redirect' param so we can send them back here after login
      nav("/login?redirect=/pricing");
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/payment/create-checkout`,
        {
          ...
          quantity: 1, // Buying 1 Subscription
        },
        {
      ...
      // 3. Redirect to Dodo Payment Page
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            ...
            <div className="bg-card p-8 rounded-2xl border border-border shadow-sm hover:border-primary/30 transition-colors text-left flex flex-col">
              ...
              <ul className="mt-6 space-y-4 text-sm text-muted-foreground mb-8">
                ...
                <li className="flex gap-2">
                  <CheckCircle2 className="w-4 h-4 text-primary shrink-0" />7
                  days retention
                </li>
                <li className="flex gap-2">
                  <CheckCircle2 className="w-4 h-4 text-primary shrink-0" />
            ...
            <div className="bg-primary p-8 rounded-2xl border border-primary shadow-xl text-left relative overflow-hidden group flex flex-col transform md:-translate-y-4">
              ...
              <div className="mt-auto">
                <button
                  ...
                  disabled={isLoading}
                  className="w-full bg-white text-primary py-3 rounded-lg font-bold hover:bg-gray-50 shadow-lg transition-colors flex items-center justify-center gap-2"
                >
                  {isLoading ? (
                    <>
            ...
            <div className="bg-card p-8 rounded-2xl border border-border shadow-sm hover:border-primary/30 transition-colors text-left flex flex-col">
              ...
              <div className="mt-auto">
                <button
                  onClick={() =>
                    (window.location.href = "mailto:sales@renard.ai")
                  }
                  className="w-full bg-secondary text-secondary-foreground py-2.5 rounded-lg font-medium hover:bg-secondary/80 transition-colors"
                >
                  Lets Talk
